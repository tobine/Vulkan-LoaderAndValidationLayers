...
through the system package manager, so applications can usually expect it to
be installed. It can also be included with an application to completely ensure
that users have a loader present.

However, the loader can also be used as a static library (this is shipped in the
...
are several downsides to this approach. The static library can never be updated
without re-linking the application. Furthermore, if the static loader is used as
part of an application which links to the loader in multiple places (such as
using two libraries which are built upon Vulkan), it is possible for the
application to link to two different versions of the loader. Objects from one
version of the loader may cause errors or crashes in the other version of the
loader. This is very unlikely with the dynamic loader, as only one copy of the
loader can be present in the system paths, with the standard name at a given
time. As a result, it is recommended that users prefer linking to the .dll and
