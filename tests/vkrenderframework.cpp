...
// Return true if layer name is found and spec+implementation values are >= reqeusted values
...
// Return true if extension name is found and spec value is >= reqeusted spec value
...
void VkRenderFramework::InitState(VkPhysicalDeviceFeatures *features, const VkCommandPoolCreateFlags flags) {
    ...
    for (auto it = m_device_extension_names.begin(); it != m_device_extension_names.end();) {
        if (!DeviceExtensionSupported(objs[0], *it)) {
            it = m_device_extension_names.erase(it);
        } else {
            ++it;
        }
    }
